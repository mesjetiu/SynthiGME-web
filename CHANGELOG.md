...
- **Zoom PiP fluido en móvil y desktop**: Auditoría completa de capas PiP. `viewportInner` ya no se redimensiona dinámicamente (tamaño fijo al natural del panel, 760×760); la escala visual la maneja `transform:scale()` en el panelEl hijo (compositor-only, sin re-layout). Eliminado layout thrashing en `refreshPipViewportMetrics` (ya no lee `clientWidth`/`clientHeight` del DOM). Quitado `will-change:transform` redundante en CSS táctil. Fusionados scroll listeners duplicados. En táctil el commit solo cambia la escala CSS interna; en desktop se redimensiona el contenedor. Fórmula de zoom amortiguada y sin mesetas. Resize por esquina con proyección diagonal continua.
