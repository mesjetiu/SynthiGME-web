...
- **Zoom PiP fluido en móvil y desktop**: El pinch-zoom (touchmove) aplica `transform: scale()` compositor-only durante el gesto. En táctil (móvil/tablet) el commit solo cambia la escala interna del panel sin redimensionar el contenedor PiP, eliminando el re-layout de ~17k nodos SVG que causaba lentitud extrema en móvil. En desktop se mantiene el resize del contenedor para ajustar el marco. Fórmula de zoom amortiguada y sin mesetas. Resize por esquina con proyección diagonal continua.
