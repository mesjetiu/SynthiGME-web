...
| `pitchToVoltageConverter.worklet.js` | Convertidor Pitch-a-Voltaje del Synthi 100 (placa PC-25). Detección de frecuencia por cruce por cero de medio ciclo con conversión logarítmica 1V/Oct (ref A4=440Hz). Track & Hold: mantiene último voltaje válido cuando la señal cae bajo el umbral de amplitud. AudioParam `rangeControl` con suavizado anti-zipper (one-pole IIR 5 Hz). Rango 250-8000 Hz. 1 entrada mono (audio), 1 salida mono (voltage CV). Mensajes: `setDormant`, `stop`. **Dormancy**: soporta `setDormant` para early exit en `process()` |
