...
### Añadido
- **Digital Sequencer 1000**: Secuenciador completo con memoria de 1024 eventos × 8 canales, FSM de transporte, clock interno 0.1–500 Hz, grabación selectiva con overdubbing, DAC 8-bit, display LED 7 segmentos, Schmitt trigger para clock externo vía matriz, 27 direcciones OSC, tooltips técnicos. Display decimal/hex configurable en Ajustes
- **Pitch to Voltage Converter (PVC)**: Detección de frecuencia por cruces por cero, conversión logarítmica 1V/Oct, track & hold, knob Range vernier piecewise de 3 segmentos. Entrada audio Panel 5, salida CV Panel 6. OSC `/pvc/range`
- **Envelope Shapers (×3)**: Generador ADSR+Delay CEM 3310 con 5 modos (Gated F/R, Free Run, Gated, Triggered, Hold), FSM de 6 fases, tiempos 1ms–20s, botón GATE + LED, señal VCA + envolvente CV ±5V. Retrigger durante release, tooltips con ΣCV e indicador de inactividad por modo
- **Ring Modulator (×3)**: Multiplicador activo sin transformador (chip 4214AP), soft-clip tanh, breakthrough −64 dB, curva LOG base 100
- **Spring Reverb**: 2 allpass + feedback loop (RT60=2.4s) + damping LPF + soft clip, mix lineal 0-10
- **Filtros LP/HP del Panel 1 (×8)**: Ladder CEM3320 con integrador TPT (Zavalishin), saturación OTA por etapa, 2× oversampling, autooscilación real en LP y HP
- **Voltímetros de Output Channels (×8)**: Aguja SVG analógica con dos modos (Signal AC / Control DC), metering zero-node integrado en worklet sin AnalyserNode, balística rápida, tooltips con Vp-p/dBm/dBFS
- **Módulo de teclado dual**: Pitch 1V/Oct, velocity ±5V proporcional, gate sin memoria. High-note priority, retrigger configurable (ON/KBD). Ventana PiP flotante con SVG, modos Normal/Latch/Legato, glissando, MIDI Learn por dispositivo+canal
- **Random Control Voltage Generator**: Reloj 0.2–20 Hz con jitter, 2 salidas DC ±2.5V (LOG), pulso Key ±5V bipolar
- **Panel 4 completo**: Blueprints de 7 columnas con PVC, Envelope Followers, Keyboards, Sequencer Output Range, Invertor/Buffer, Slew Limiters
- **MIDI Learn universal**: Mapeo CC/Note/PitchBend para cualquier control, persistencia, exportar/importar JSON, pestaña en Ajustes
- **Modo de renderizado (GPU/CPU)**: Detección automática de GPU, 3 modos (Auto/Calidad/Rendimiento), ~60 selectores CSS optimizados para rendimiento
- **Instrumentación de rendimiento UI**: Monitor runtime `?perf=1`, script `npm run perf:ui` con Chromium + CDP
- **Notas post-it en paneles**: Arrastrables, editables, texto enriquecido, copiar/pegar entre paneles, 6 colores, persistencia
- **Panel 4 blueprint v2**: Estructura visual completa con voltímetros, display Sequencer Event Time y 7 columnas de Keyboard/Sequencer Output Range
- **Undo/Redo global**: Snapshots (hasta 50 estados), Ctrl+Z/Ctrl+U
- **Selector rotativo (RotarySwitch)**: Componente UI fiel al hardware para switches de 2 posiciones
- **Easter egg**: Desintegración visual + pieza electroacústica al completar secuencia en joystick

### Cambiado
- **Valores hardcodeados extraídos a config**: 6 módulos (sequencer, keyboard, envelope shaper, random CV, ring modulator, spring reverb) reciben parámetros vía `processorOptions`
- **Sequencer clock vía matriz**: El reloj solo genera pulso; requiere patch fila 88 → col 51 como en el hardware real
- **Filtros LP/HP: integrador TPT**: Reescrito DSP con pre-warping Zavalishin, saturación OTA CEM3320, HP por sustracción binomial
- **Paneles flotantes (PiP) rediseñados**: Sin marco, preview estructural durante interacción, zoom unificado (pinch/resize/Ctrl±), re-rasterización nítida tras zoom, paneo y zoom coalescidos a 1 rAF/frame, contención CSS, presupuesto will-change dinámico en Firefox
- **Canvas principal fijo en overview**: Detach/return por doble clic, atajos 1-7/0/9/8, resize proporcional, PiP siempre parcialmente visible, zoom out sin tope
- **Tooltips unificados**: Todos los controles usan tooltip estilizado `knob-tooltip` con nombre del control como prefijo. API genérica `attachControlTooltip()`
- **Panel 3 y knobs rasterizados**: Fondos WebP, knobs/vernier/toggles/rotary como PNG pre-rasterizado, ~17k nodos DOM eliminados
- **Output Channel sliders**: `shellWidth` independiente, layout vertical corregido
- **knobColors en blueprints**: Colores originales declarados en paneles 2, 3 y 7
- **MODULE-CREATION-GUIDE.md ampliada**: Patrones de Sequencer y Keyboard, ~23 archivos checklist

### Corregido
- **Osciladores: zipper noise en Firefox**: Suavizado one-pole IIR per-sample en todos los AudioParam (cutoff 5Hz, τ≈32ms)
- **Voltímetros: audio se cortaba al mover knobs**: Eliminados AnalyserNode y lecturas síncronas; metering zero-node vía postMessage desde worklet
- **Envelope Shaper**: KEY CV conectado al trigger real, retrigger durante release (CEM 3310), ruteo P5/P6 completo, selector mode en arco 10–2 o'clock, keepalive permanente sin dormancy
- **Secuenciador**: 6 bugs de comunicación worklet/UI, clock rate vernier real, tooltips conectados con valores escalados, reinicio contextual completo, display y callbacks por orden de inicialización, Schmitt trigger con blanking contra ringing
- **Teclados**: Escalado pitch/velocity/gate corregido (÷DIGITAL_TO_VOLTAGE), retrigger según manual, notas SVG despachadas, glissando con interpolación
- **Random CV**: Escalado de salidas V1/V2 y Key corregido (÷DIGITAL_TO_VOLTAGE), pulso key espurio al despertar de dormancy
- **knobTypes Panel 3**: Noise Colour y Random CV Voltage/Key con tipo correcto (normal/bipolar)
- **PiP**: Matrices al tamaño correcto tras fullscreen, toggles SVG estables, pinch-zoom tablet sin saltos, zoom out restaurado, offset libre
- **Vernier**: Eliminado "0" fantasma en vernier-ring.png
- **Rendimiento Firefox**: Eliminado `cancelScheduledValues` innecesario, will-change dinámico en knobs, scheduling condicional en setFrequency

### Mejorado
- **Rendimiento de knobs en Firefox**: Eliminado layout thrashing, badge lazy, tooltip ruta rápida textContent
- **Rendimiento de audio en Firefox**: setTargetAtTime sin cancel previo, scheduling condicional por salto relativo
- **Modo rendimiento CSS ampliado**: Animaciones, box-shadow, transitions, backdrop-filter desactivados; Firefox auto-detectado
- **Tipografía histórica**: Microgramma web real en Panel 3, scripts de knobs con fuentes locales explícitas
- **4393 tests** (config, worklet, módulo, OSC, blueprint, dormancy, tooltip, UI, audio, integración)
