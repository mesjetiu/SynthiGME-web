...
| `pitchToVoltageConverter.worklet.js` | Convertidor Pitch-a-Voltaje del Synthi 100 (placa PC-25). Detección de frecuencia por cruce por cero de medio ciclo con conversión logarítmica 1V/Oct (ref A4=440Hz). Track & Hold: mantiene último voltaje válido cuando la señal cae bajo el umbral de amplitud. Sin AudioParams: el dial Range llega por mensaje `setRange` (dial 0-10 → spread −2..+2, codos en 3,5 y 7). Sin interpolación de cruces: la lectura se cuantiza a muestras enteras de semiperiodo (±32 cents a 440 Hz, ±74 a 1 kHz). Rango 250-8000 Hz. 1 entrada mono (audio), 1 salida mono (voltage CV). Mensajes: `setRange`, `setDormant`, `stop`. **Dormancy**: soporta `setDormant` para early exit en `process()` |
...
| `pitchToVoltageConverter.js` | `PitchToVoltageConverterModule` | Convertidor Pitch-a-Voltaje del Synthi 100 (placa PC-25). 1 instancia en Panel 4. Cadena: `inputGain(=1) → AudioWorkletNode(pitch-to-voltage-converter, 1in/1out) → outputGain(=1)`. Keepalive GainNode (gain=0→destination). 1 knob: range (vernier, 0-10, piecewise 3 segmentos). Entrada audio Panel 5 col 50, salida CV Panel 6 fila 121. Lazy start al primer pin. Dormancy: envía `setDormant` al worklet |
...
| `pitchToVoltageConverter.config.js` | Pitch to Voltage Converter | 1 instancia, 1 knob range (vernier, 0-10, piecewise 3 segmentos). Rango frecuencia 250-8000 Hz, umbral amplitud 0.02, 1V/Oct. Panel 5 col 50 (entrada audio), Panel 6 fila 121 (salida CV) |
...
- **PitchToVoltageConverter dormant**: mensaje `setDormant` al worklet → early exit en `process()`. Detecta conexiones en Panel 5 col 50 (entrada) y Panel 6 fila 121 (salida).
...
| 50 | Pitch to Voltage Converter | Entrada de audio para detección de frecuencia (1V/Oct). Señal → worklet → voltaje CV en Panel 6 fila 121. |
...
- **Pitch to Voltage Converter** (fila 121): salida CV 1V/Oct desde detección de frecuencia en Panel 5 col 50
...
│   ├── recordingCapture.worklet.test.js # Worklet recording-capture real: start/stop, un samples por bloque con copia transferida, silencio en canales ausentes
