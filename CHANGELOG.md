...
### Añadido
- **Knobs SVG del Synthi 100 original**: Nuevo diseño visual basado en SVG con anillo exterior que gira (escala 0-10, ticks, muescas) y centro de color fijo con brillo simulado. Colores del original: azul (pulse level, triangle, filter, colour), verde (pulse shape, sine symmetry), blanco (sine level, saw, pan, input amplifiers, voltage, key, noise level), negro (frequency), rojo (mean, variance), amarillo (joystick range). Rango de giro 300° (10/12 de circunferencia). Diseño CSS anterior conservado como fallback.
