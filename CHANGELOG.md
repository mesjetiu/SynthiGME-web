...
### Cambiado
- **Filtros LP/HP: modelo Huovilainen (CEM3320 OTA)**: Reescrito el núcleo DSP del ladder con el modelo Huovilainen: cada etapa aplica `tanh` (saturación OTA del CEM3320), lo que limita la amplitud de forma natural y produce autooscilación estable a Q alto. Ruido blanco inaudible (0.001, ~-60 dBFS) a la entrada como semilla; coeficiente integrador exponencial (`g = 1 − e^(−2π·fc/fs)`); curva de realimentación ampliada (3.95 + t·1.05, máx. 5.0) para inicio fiable de oscilación desde dial ≈7. Eliminada la generación artificial de sinusoide y los estados `selfPhase`/`in1–in4`.
...
- **Zoom negativo de PiP limitado al tamaño de origen**: Los paneles flotantes ya no pueden reducirse por zoom por debajo de su tamaño inicial por defecto. El zoom out vuelve exactamente a esa geometría base, evitando miniaturas excesivamente pequeñas incluso al restaurar PiPs guardadas de sesiones anteriores. El límite mínimo se recalcula dinámicamente según el tamaño actual de la ventana del navegador, en vez de quedar fijado al tamaño que tenía cuando se creó el PiP.
- **Resize PiP respeta límite mínimo de zoom**: Arrastrar los bordes o esquinas de una ventana flotante ya no permite reducirla por debajo de la escala mínima. Anteriormente solo `MIN_PIP_SIZE` (150px) actuaba de suelo; ahora las dimensiones mínimas se derivan de `getMinScale()` (idéntico al Ctrl+rueda/Ctrl+-), evitando que el fondo se recorte al encoger demasiado.
- **Animación de devolución de PiP sin contenido cortado**: Al devolver un panel flotante al canvas, la escala interna (`transform: scale`) se mantiene durante la animación de salida y solo se elimina al completarse. Anteriormente se reseteaba antes de animar, provocando que el contenido apareciera a tamaño completo (grande y cortado) dentro del contenedor que se estaba reduciendo.
