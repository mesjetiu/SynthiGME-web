...
- **Pinch-zoom PiP fluido en móvil/tablet**: Durante el gesto de pellizco, el transform visual se aplica directamente en touchmove (sin RAF intermedio) usando solo `transform: scale() translate3d()` compositor-only, y el resize real se commitea una sola vez al soltar. Se eliminan también las llamadas repetidas a `setPipPreviewMode` y `getBoundingClientRect` que forzaban layout/estilo en cada touchmove.
