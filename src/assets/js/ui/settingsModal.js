...
export class SettingsModal {
  ...
  constructor(options = {}) {
    ...
    this.panelResolutions = savedPanelResolutions ? JSON.parse(savedPanelResolutions) : {};
  ...
  _createResolutionSection() {
    ...
    // Sección colapsable de resolución por panel
    section.appendChild(this._createPanelResolutionSubsection());
    
    return section;
  }
  
  /**
   * Crea la subsección de resolución por panel (colapsable)
   * @returns {HTMLElement}
   */
  _createPanelResolutionSubsection() {
    const subsection = document.createElement('div');
    subsection.className = 'settings-subsection settings-subsection--collapsible';
    ...
    this.panelResolutionHeaderElement = document.createElement('span');
    this.panelResolutionHeaderElement.textContent = t('settings.scale.perPanel');
    
    const arrow = document.createElement('span');
    arrow.className = 'settings-subsection__arrow';
    arrow.innerHTML = '&#9660;'; // ▼
    
    header.appendChild(this.panelResolutionHeaderElement);
    header.appendChild(arrow);
    
    // Contenido colapsable
    const content = document.createElement('div');
    content.className = 'settings-subsection__content';
    
    this.panelResolutionDescElement = document.createElement('p');
    this.panelResolutionDescElement.className = 'settings-section__description';
    this.panelResolutionDescElement.textContent = t('settings.scale.perPanel.description');
    content.appendChild(this.panelResolutionDescElement);
    
    // Crear selector para cada panel (1-6)
    const panelNames = {
      'panel-1': t('settings.scale.panel.1'),
      'panel-2': t('settings.scale.panel.2'),
      'panel-3': t('settings.scale.panel.3'),
      'panel-4': t('settings.scale.panel.4'),
      'panel-5': t('settings.scale.panel.5'),
      'panel-6': t('settings.scale.panel.6')
    };
    
    // Paneles marcados por defecto (abiertos al expandir)
    const highlightedPanels = ['panel-5', 'panel-6'];
    
    this.panelResolutionSelects = {};
    
    Object.entries(panelNames).forEach(([panelId, panelName]) => {
      const row = document.createElement('div');
      row.className = 'settings-row settings-row--select settings-row--panel-resolution';
      if (highlightedPanels.includes(panelId)) {
        row.classList.add('settings-row--highlighted');
      }
      
      const label = document.createElement('label');
      label.className = 'settings-row__label';
      label.textContent = panelName;
      label.setAttribute('for', `resolution-${panelId}`);
      
      const select = document.createElement('select');
      select.id = `resolution-${panelId}`;
      select.className = 'settings-select settings-select--small';
      
      // Opción "Global"
      const globalOption = document.createElement('option');
      globalOption.value = 'global';
      globalOption.textContent = t('settings.scale.perPanel.global');
      ...
      // Valor actual
      const currentValue = this.panelResolutions[panelId];
      select.value = currentValue !== undefined ? currentValue : 'global';
      
      select.addEventListener('change', () => {
        this._setPanelResolution(panelId, select.value);
      });
      
      this.panelResolutionSelects[panelId] = select;
    ...
    header.addEventListener('click', () => {
      ...
      subsection.classList.toggle('settings-subsection--expanded', isExpanded);
  ...
  /**
   * Establece la resolución para un panel específico
   * @param {string} panelId
   * @param {string} value - 'global' o factor numérico
   */
  _setPanelResolution(panelId, value) {
    if (value === 'global') {
      delete this.panelResolutions[panelId];
    } else {
      const factor = parseFloat(value);
      this.panelResolutions[panelId] = factor;
      
      // Warning si es móvil y factor > threshold
      if (this.isMobile && factor > this.mobileWarningThreshold) {
        this._showToast(t('toast.resolution.panelMobileWarning', { panel: panelId.replace('panel-', ''), factor }));
      }
    }
    
    // Guardar en localStorage (siempre, independiente de "recordar")
    localStorage.setItem(STORAGE_KEYS.PANEL_RESOLUTIONS, JSON.stringify(this.panelResolutions));
    
    // Notificar al sistema de navegación
    if (typeof window.__synthSetPanelResolutions === 'function') {
      window.__synthSetPanelResolutions(this.panelResolutions);
    }
  }
