...
- **Pinch-zoom PiP fluido en móvil/tablet**: El gesto de pellizco con dos dedos en paneles flotantes se batcha ahora a un único `requestAnimationFrame` por frame visual, en lugar de ejecutar layout síncrono en cada evento `touchmove`. Elimina reflows forzados múltiples por frame (getBoundingClientRect + style writes) que causaban saltos visibles a 60-120Hz.
