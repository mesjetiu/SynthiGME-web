/**
 * Panel1ReverbUI — UI de Reverberación de Muelle para Panel 1
 *
 * Estructura visual: ModuleFrame con 2 knobs (Mix, Level).
 * Sigue el mismo patrón que Panel1FilterUI.
 *
 * @version 1.0.0
 */
...
export class Panel1ReverbUI {
  constructor(options = {}) {
    this.id = options.id;
    this.cssClass = options.cssClass || 'panel1-reverb-live';
    this.layout = {
      knobGap: options.knobGap ?? 6,
      knobSize: options.knobSize ?? 65,
      knobInnerPct: options.knobInnerPct ?? 78,
      knobColors: options.knobColors || ['blue', 'white'],
      knobTypes: options.knobTypes || ['normal', 'normal'],
  ...
  createElement() {
    this.frame = new ModuleFrame({
      id: this.id,
      title: null,
      className: `panel1-placeholder panel1-reverb ${this.cssClass}`
    });
