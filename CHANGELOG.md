...
- **Zoom PiP fluido en móvil y desktop**: El pinch-zoom (touchmove) y el zoom con rueda/touchpad (Ctrl+scroll) aplican `transform: scale()` compositor-only durante el gesto. En tablet, el commit solo cambia la escala interna del panel (sin resize del contenedor PiP), eliminando el re-layout de ~17k nodos que causaba flashes blancos en paneles pesados como Panel 3. En desktop se mantiene el resize del contenedor para ajustar el marco. `ensurePanelMetrics` usa valores cacheados para evitar `offsetWidth` entre escritura de transform y escritura de tamaño. Capa GPU dedicada en `.pip-viewport-inner` solo en `pointer: coarse` para evitar borrosidad en desktop.
...
- **PiP tablet: salto de posición y zoom no uniforme en pinch**: El pinch-zoom en PiP de tablet tenía tres problemas: (1) al soltar los dedos la ventana saltaba porque el commit no compensaba el efecto visual del scale-around-origin del preview, (2) el preview permitía escalas que el commit revertía al soltar, y (3) existía una meseta dura al acercar mucho los dedos (`PIP_MIN_PINCH_DIST`) que hacía que el zoom pareciera avanzar por etapas o quedarse fijo mientras seguía entrando pan lateral. Corregido igualando los límites visuales y reales, sincronizando el commit con la escala realmente mostrada en preview y sustituyendo el umbral duro por una amortiguación continua.

### Cambiado
- **Panel 3 y knobs estándar rasterizados**: El fondo de Panel 3 pasa a imagen WebP en lugar de SVG inline para eliminar el coste de ~17k nodos en el DOM. Los knobs giratorios que usaban `knob.svg` y `knob-0-center.svg` mantienen la misma interacción, pero su anillo exterior se carga ahora como imagen PNG reutilizable con transparencia alfa completa (fondo y zona de cifras); el vernier multivuelta sigue en SVG.
