...
- **Knobs SVG del Synthi 100 original**: Diseño visual basado en SVG inline inyectado programáticamente con anillo exterior que gira (escala 0-10, ticks, muescas) y centro de color integrado en el propio SVG (eliminado el div `.knob-center` y la variable CSS `--knob-center-color`). Color del centro se inyecta via `setAttribute('fill', color)` sobre el elemento `#knob-center-color` del SVG. IDs únicos por instancia para evitar colisiones DOM. Nuevo módulo `svgInlineLoader.js` compartido (fetch+cache+prefijo de IDs). Colores del original: azul, verde, blanco, negro, rojo, amarillo. Rango de giro 300°. Diseño CSS anterior conservado como fallback.
...
- **Colores de centro de knobs centralizados**: `configs/knobColors.js` como fuente única de verdad. Color inyectado programáticamente en el SVG inline (sin CSS `--knob-center-color` ni divs `.knob-center`). Eliminados archivos `centro-knob-*.svg` individuales por color.
