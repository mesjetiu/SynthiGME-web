...
- **Pinch-zoom PiP fluido en móvil/tablet**: Durante el gesto de pellizco, el transform visual se aplica directamente en touchmove (solo `transform: scale() translate3d()` compositor-only). La posición final se calcula desde la geometría del preview (sin usar `applyPipWheelZoom`), eliminando el desplazamiento que quedaba al soltar los dedos. La capa GPU (`will-change: transform`) se mantiene permanente en `.pip-container` vía CSS, eliminando el coste de promoción on-the-fly que causaba lag en los primeros frames del gesto.
