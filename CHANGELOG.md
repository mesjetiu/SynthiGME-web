...
- **Patches: configuración visual completa (PIPs + viewport)**: Checkbox "Incluir configuración visual" en el Patch Browser permite guardar y restaurar la disposición de paneles flotantes y la posición/zoom del canvas principal junto con el patch. Al guardar, se serializa el estado de PIPs (posición, tamaño, zoom, scroll, lock) y el viewport (escala, desplazamiento, panel enfocado). Al cargar, se restaura automáticamente. Icono 🖼 indica patches con layout visual. Nuevo ajuste en Interfaz para recordar la posición del viewport entre sesiones (independiente del ajuste de PIPs). Traducciones en 7 idiomas.
