...
- **Re-entry de Output Channels: drift sub-Hz y fidelidad de forma de onda**: Historial completo: v0.5 usaba DC blocker 0.01 Hz (settling ~80s) causando drift audible de pitch (~5 cents/mV) al usar re-entry como CV para FM. v0.6-rc1 elevó a 2 Hz para settling rápido (~80ms) pero destruyó LFOs (<2 Hz): onda cuadrada 1 Hz tenía 99.8% droop en tramos planos, inusable como CV. Solución definitiva en v0.6: revertir a 0.01 Hz + hard-set (`setValueAtTime` tras 5τ) en crossfade del filter bypass. El hard-set elimina el residuo asintótico de `setTargetAtTime` que era la causa real del drift, sin necesidad de subir la frecuencia del DC blocker. A 0.01 Hz: droop de cuadrada 1 Hz es solo ~3% (<0.3 dB), transparente como CV para FM. Crossfade fuerza valores exactos (0 cuando bypassed), sin fugas.
