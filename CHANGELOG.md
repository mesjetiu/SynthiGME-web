...
- **Electron: menú nativo desaparecía tras pantalla completa**: Al salir de fullscreen (F11 o menú), `autoHideMenuBar` quedaba activado internamente por Chromium, ocultando el menú permanentemente. Forzado `setAutoHideMenuBar(false)` y `setMenuBarVisibility(true)` en el evento `leave-html-full-screen`.
