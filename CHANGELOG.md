...
- **Zoom PiP fluido en móvil y desktop**: `viewportInner` se redimensiona al tamaño visual del panel (`panelWidth * scale`) para que la textura GPU sea proporcional al zoom (~3MB a scale 0.4 vs ~20MB a 760px fijo en 3× DPR). `panelEl` tiene `width`/`height` fijos para que los cambios de `viewportInner` no provoquen re-layout del SVG. Eliminado `setPipPreviewMode` espurio en touchstart. Eliminado layout thrashing en `refreshPipViewportMetrics` (calcula desde `state.width`). Fusionados scroll listeners duplicados. En táctil el commit solo cambia la escala CSS interna; en desktop se redimensiona el contenedor. Fórmula de zoom amortiguada y sin mesetas. Resize por esquina con proyección diagonal continua.
