...
- **Notas post-it en paneles**: Notas arrastrables y editables sobre cualquier panel (canvas y PiP). Arrastre entre paneles y al espacio libre del viewport (notas sin panel). Texto enriquecido (negrita/cursiva con Ctrl+B/I). Alineación izquierda/centrado/derecha en menú contextual. Menú contextual propio (cortar/copiar/pegar texto, formato, alineación). Copiar/cortar/pegar notas entre paneles. Botones A+/A- para tamaño de fuente. 6 colores seleccionables. Resize hasta 50×36px. Persistencia en localStorage y patches. Eventos aislados (no traspasan al panel). 100 tests.
