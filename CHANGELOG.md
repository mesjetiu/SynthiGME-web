...
- **Notas post-it en paneles**: Notas arrastrables y editables sobre cualquier panel (canvas y PiP). Texto enriquecido (negrita/cursiva con Ctrl+B/I). Menú contextual propio (cortar/copiar/pegar texto, negrita, cursiva). Copiar/cortar/pegar notas entre paneles. Botones A+/A- para tamaño de fuente. 6 colores seleccionables. Persistencia en localStorage y patches. Eventos aislados (no traspasan al panel). 82 tests.
