...
### Cambiado
- **Filtros LP/HP: modelo Huovilainen (CEM3320 OTA)**: Reescrito el núcleo DSP del ladder con el modelo Huovilainen: cada etapa aplica `tanh` (saturación OTA del CEM3320), lo que limita la amplitud de forma natural y produce autooscilación estable a Q alto. 2x oversampling para preservar resonancia a frecuencias altas (sin él, g→1 y el Q colapsa). HP derivado del mismo ladder por sustracción binomial [1,−4,6,−4,1], con resonancia y autooscilación reales en lugar de cascada HP separada. Level en outputGain (post-filtro), silenciando autooscilación con level=0. Ruido blanco inaudible (0.001) a la entrada como semilla; curva de realimentación ampliada (3.95 + t·1.05, máx. 5.0).
