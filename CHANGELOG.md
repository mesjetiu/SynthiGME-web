...
- **Zoom PiP fluido en móvil y desktop**: El pinch-zoom (touchmove) y el zoom con rueda/touchpad (Ctrl+scroll) aplican ahora solo `transform: scale()` compositor-only durante el gesto, sin manipular `width/height/left/top` ni leer layout (`offsetWidth`, `getBoundingClientRect`). El resize real se hace una sola vez al final del gesto (touchend / fin de scroll). Se pre-calculan los límites de escala en touchstart/primer wheel para eliminar lecturas de layout durante el gesto. Eliminadas capas GPU intermedias (`translateZ(0)`, `will-change: transform` en `.pip-viewport-inner` y `.pip-container`) que impedían que el modo nítido (`CSS zoom`) re-rasterizara el panel a mayor resolución, causando borrosidad a alto zoom en desktop.
