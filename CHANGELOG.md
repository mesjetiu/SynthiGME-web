...
### Añadido
- **Easter egg**: Desintegración visual del Synthi (knobs, sliders, módulos flotan, giran y se disuelven con paleta sombría-colorida) + pieza musical al completar secuencia de 8 taps alternados en joystick pads (L,R,L,R…). Dos piezas: electroacústica ("Studie") y chiptune 8-bit. Modo solo-visual si la sesión tiene cambios (isDirty capturado al inicio de la secuencia, markClean al completar para deshacer dirty de los propios taps). Sin dependencias externas (Web Animations API + DOM). 98 tests.
