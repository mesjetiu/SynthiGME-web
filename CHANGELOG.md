...
- **Patches: configuración visual opcional (PIPs)**: Checkbox "Incluir configuración visual (PIPs)" en el Patch Browser permite guardar y restaurar la disposición de paneles flotantes junto con el patch. Al guardar con la opción activa, se serializa el estado de todas las PIPs (posición, tamaño, zoom, scroll, lock). Al cargar, se restaura automáticamente. Icono 🖼 en la lista indica patches con layout visual incluido. Preferencia persistente en localStorage. Compatible con import/export de archivos `.sgme.json`. Traducciones en 7 idiomas.
