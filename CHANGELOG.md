...
- **Output Channel: DC blocker eliminado de la re-entry**: El DC blocker (AudioWorklet 1er orden, 0.01 Hz) en la ruta de re-entrada a la matriz destruía señales DC legítimas (joystick, voltajes de control constantes), haciendo que cualquier voltaje DC tendiera suavemente a 0V. Eliminado completamente: `postVcaNode` es el punto directo de re-entry, se eliminó el alias `dcBlocker`, el archivo `dcBlocker.worklet.js` y todas las referencias.
