...
### Añadido
- **MIDI Learn**: Sistema completo de mapeo MIDI para cualquier control del sintetizador (knobs, sliders, toggles). Menú contextual con "MIDI Learn" / "Quitar MIDI" en cada control. Soporta CC, Note On/Off y Pitch Bend, todos los dispositivos activos simultáneamente, anti-feedback (patrón OSC sync). Persistencia en localStorage + exportar/importar JSON. Pestaña MIDI en Ajustes con dispositivos detectados, tabla de mappings y acciones. Overlay visual durante learn con estado de dispositivos. Apertura explícita de puertos MIDI (imprescindible en Linux). Logging con `console.log` para visibilidad en builds de producción. Traducciones en 7 idiomas. 42 tests unitarios (parsing, claves, conversión, mappings).
